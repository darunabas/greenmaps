...
get_species_richness_data <- function(species_data, ras, tdwg, mlist,
  ...
  z <- setValues(ras, m$richness[index])
  z[z <= 0] <- NA
  names(z) <- "richness"
  project(z, "EPSG:4326")
}
...
extract_biodiversity_data <- function(species_data, ras, tdwg, mlist,
  ...
  validate(
    need(geoscale == "all" || !(is.null(region_name) || region_name == ""),
         "Region name not selected"),
    need(taxo_rank == "all" || !(is.null(taxon) || taxon == ""),
  ...
  validate(
    need(nrow(species_data) > 0, "No biodiversity found for selected filters")
  )
